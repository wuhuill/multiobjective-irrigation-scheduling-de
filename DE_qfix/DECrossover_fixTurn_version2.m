function crossedPopulation = DECrossover_fixTurn_version2(mutatedPopulation, parentPopulation, Cr, params)
...
function individual = fixStageLengthsAndIntervals(individual, params)
    % 修复轮期长度和间隔的约束
    individual.stageLengths = min(max(individual.stageLengths, 15), 45); % 确保最小长度为10
...
function crossedIndividual= CrossIrrigationStartTime(crossedIndividual, mutatedIndividual, IrrigationLength, params)
    for stage = 1:params.numStages
        ...
        for k = 1:params.I
            % 当前区域的时间段
            currStart = crossedIndividual.irrigationStart(k, stage);
            currEnd = crossedIndividual.irrigationEnd(k, stage);
            ...
            for m = 1:params.I
                if m ~= k
                    % 其他区域的时间段
                    otherStart = crossedIndividual.irrigationStart(m, stage);
                    otherEnd = crossedIndividual.irrigationEnd(m, stage);
                ...
                for m = 1:params.I
                    if m ~= k
                        ...
                        if currStart > otherEnd
                            % 如果当前时间段在其他区域之后，向前移动
                            crossedIndividual.irrigationStart(k, stage) = otherEnd - IrrigationLength(k) + 1;
                            crossedIndividual.irrigationEnd(k, stage) = otherEnd;
                        elseif currEnd < otherStart
                            % 如果当前时间段在其他区域之前，向后移动
                            crossedIndividual.irrigationStart(k, stage) = otherStart;
                            crossedIndividual.irrigationEnd(k, stage) = otherStart + IrrigationLength(k) - 1;
                        end
                        
                        % 检查调整后的合法性
                        crossedIndividual.irrigationStart(k, stage) = max(crossedIndividual.irrigationStart(k, stage), stageStartJ);
                        crossedIndividual.irrigationEnd(k, stage) = min(crossedIndividual.irrigationEnd(k, stage), stageEndJ);
        ...
        newStageEnd = max(crossedIndividual.irrigationEnd(:, stage));
        crossedIndividual.stageEnd(stage) = newStageEnd;

        % 更新轮期长度
        crossedIndividual.stageLengths(stage) = newStageEnd - stageStartJ + 1;

        % 如果需要更新轮期间隔，确保至少存在下一轮期
        if stage < params.numStages
            nextStageStart = crossedIndividual.stageStart(stage + 1);
            crossedIndividual.stageIntervals(stage) = nextStageStart - newStageEnd;
        end
    end
