...
%    end
   ...
   for j = 1:length(v)
       for i = 1:I
           [wheat_Yield_unit(j, i), ~] = Crop_Yield(IQwheat(j, i, :),Temp_wheat,RN_wheat,EP_wheat,ET0_wheat,start_time_wheat,end_time_wheat,Tb(1),PHU(1),TO(1),ab1(1),ab2(1),LAImax(1),HUI0(1),ad(1),BE(1),HI(1),Kc_wheat,torigin(1),tending(1),Rootorigin(1),Rootending(1),thetaorigin,fieldcapacity,wilting);
           [maize_Yield_unit(j, i), ~] = Crop_Yield(IQmaize(j, i, :),Temp_maize,RN_maize,EP_maize,ET0_maize,start_time_maize,end_time_maize,Tb(2),PHU(2),TO(2),ab1(2),ab2(2),LAImax(2),HUI0(2),ad(2),BE(2),HI(2),Kc_maize,torigin(2),tending(2),Rootorigin(2),Rootending(2),thetaorigin,fieldcapacity,wilting);
