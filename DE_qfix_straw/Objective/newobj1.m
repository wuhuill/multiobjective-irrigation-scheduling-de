...
for j = 1:length(v)
    for i = 1:I
        ...
        wheat_straw (j, i) = wheat_straw_unit(j, i) * Area_wheat(i);
        maize_straw (j, i) = maize_straw_unit(j, i) * Area_maize(i);
...
wheat_straw = sum(wheat_straw, 2);
maize_straw = sum(maize_straw, 2);

Yield_total = sum(wheat_Yield, 2) + sum(maize_Yield, 2);
% 秸秆的热量
f1 = 14 * wheat_straw + 19 * maize_straw; 
f1 = - f1;  
   ...
   maxWaterPerStage = params.maxWaterPerStage_all;
   for i = 1:length(v)
